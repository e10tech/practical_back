...
#この関数は、新規でデータを挿入するためのもの
# myinsert関数は、指定されたモデルに対して新しいデータを挿入します。
# 引数には、挿入するモデルと値の辞書を受け取ります。
...
def myupdate(mymodel, values):
    # session構築
    # SQLAlchemyのセッションを作成するためのクラスを生成
    # engine（DB接続情報）を使ってSessionクラスを作る
    Session = sessionmaker(bind=engine)
    # 実際にDB操作を行うためのセッションインスタンスを作成
    session = Session()

    # 更新対象の顧客ID（customer_id）をvaluesから取り出す
    # pop()を使うことで、customer_idの値を取得しつつ、values辞書からcustomer_idキーを削除します
    # これは、updateクエリのwhere句で使うためです
